...
class MonitoringApp(ctk.CTk):
    def __init__(self):
        ...
        # Read credentials and create tabs
        ...
        try:
            with open("credentials.py", "r") as file:
                ...
                if not ip_addresses:
                    ...
                    message_label.grid(row=0, column=0, pady=20, sticky="nsew")  # Using grid instead of pack
                ...
                for ip_address in ip_addresses:
                    ...
                    # Fetch data from URL and update periodically
                    self.update_data(ip_address)

        except FileNotFoundError:
            error_label = ctk.CTkLabel(self.tab_view, text="Credentials file not found!")
            error_label.grid(row=0, column=0, pady=20, sticky="nsew")  # Using grid instead of pack
    ...
    def update_data(self, ip_address):
        try:
            ...
            # Display response content in the scrollable text area
            self.text_areas[ip_address].delete("1.0", "end")
            self.text_areas[ip_address].insert("1.0", response.text)
    ...
    def toggle_switch(self, ip_address):
        try:
            ...
            # Check response content for "was_on" value
            ...
            error_msg = f"Error toggling switch for {ip_address}: {e}"
            error_label = ctk.CTkLabel(self.tab_view, text=error_msg)
            error_label.pack(pady=20)
            logging.error(error_msg)
        ...
        try:
            ...
            def update_chart():
                try:
                    ...
                    chart_window.after(5000, update_chart)
            ...
            chart_window.title(f"Power Chart - {ip_address}")
            chart_frame = ctk.CTkFrame(chart_window)
            chart_frame.pack(side='left', expand=True, fill='both', ipady=5, ipadx=5, padx=5, pady=5)
